#!/bin/bash
cat << 'INNER_EOF' > internal/storage/sqlite.go
package storage
	...
	for _, db := range dbs {
		...
		for rows.Next() {
			...
			if svc, ok := serviceMap[name]; ok {
				svc.SpanCount += int(spanCount)
				svc.ErrorCount += int(errorCount)
...
func (s *SQLiteStorage) GetStats(ctx context.Context, sinceNano int64) (*Stats, error) {
	...
	dbs := s.getAllDBs()
	for _, db := range dbs {
		var traces, spans, errors int64
		var sumDuration sql.NullFloat64
		err := db.QueryRowContext(ctx, `
			...
			Scan(&traces, &spans, &sumDuration, &errors)
		if err != nil {
	...
	stats.ServiceCount = int64(len(serviceSet))
...
func (s *SQLiteStorage) DeleteOlderThan(ctx context.Context, beforeNano int64) (int64, error) {
	// For partitioned DBs, we can just delete whole files that are old enough!
	// Here we implement file deletion for entire partitions, plus standard rows deletion 
	// for files that might still contain mixed records (though partitioned by day, 
	// there might be some overlap at boundaries).
	...
	beforeTime := time.Unix(0, beforeNano)
