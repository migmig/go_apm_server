...
- [x] **Task 17-6: Exemplar 통합 테스트 (Test)**
...
- [x] **Task 19-1: Semantic Conventions 스키마 승격 (Backend)**
  ...
  - DB 마이그레이션 실행 전 `apm.db` 백업 스크립트 작성 (`scripts/backup_db.sh`)

- [x] **Task 19-2: 기존 데이터 백필 마이그레이션 (Backend)**
  - `json_extract` 활용하여 기존 `attributes` JSON에서 승격 대상 키 값을 신규 컬럼에 역채움 (`BackfillSpanSemanticColumns`)
  - DB 잠금 방지를 위한 배치 단위(1000건) 분할 실행 로직 구현
  - 백필 진행률 로깅 (처리 건수 / 전체 건수)

- [x] **Task 19-3: 데이터 파싱 최적화 (Backend)**
...
- [x] **Task 19-4: API 고급 검색 쿼리 연동 (API)**
...
- [x] **Task 19-5: 시맨틱 통합 필터 (Frontend)**
...
- [x] **Task 19-6: 성능 벤치마크 및 테스트 (Test)**
...
- [x] **Task 18-1: Exporter Config 및 워커 큐 프로토콜 설계 (Backend)**
...
- [x] **Task 18-2: 멀티 시그널 OTLP Client 송신 로직 구현 (Backend)**
  - 시그널별 OTLP Client 구현:
...
- [x] **Task 18-6: Exporter 상태 모니터링 API (API)**
  - `GET /api/exporter/status` 엔드포인트 구현
  - 엔드포인트별 연결 상태, Circuit Breaker 상태, DLQ 파일 개수 실시간 반환

- [x] **Task 18-7: Exporter 통합 테스트 (Test)**
  - 멀티 시그널 팬아웃 통합 테스트: `internal/exporter/exporter_test.go`
  - Circuit Breaker 상태 전이 테스트 (Closed → Open → Half-Open → Closed)
  - DLQ 저장/복구 통합 테스트
  - Exporter 상태 API 응답 검증

- [x] **Task 18-8: 앱 릴리즈 및 문서화 (Docs)**
