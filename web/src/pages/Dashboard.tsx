...
export default function Dashboard() {
  ...
  const fetchData = useCallback(async (backgroundRefresh = false) => {
    ...
    try {
      ...
      const msg = getErrorMessage(err, '대시보드 데이터를 불러오지 못했습니다. API 서버 연결을 확인해 주세요.');
      if (backgroundRefresh) {
        toast.error(msg, { id: 'dashboard-poll' });
      } else {
        setErrorMessage(msg);
      }
    } finally {
  ...
  const statCards = useMemo(
    () => {
      ...
      return [
        { label: '전체 요청 수', value: formatCount(stats?.total_traces), icon: Activity, colorClass: 'text-blue-400', bgClass: 'bg-blue-500/10', dataKey: 'rps' },
        { label: '전체 작업(Span) 수', value: formatCount(stats?.total_spans), icon: Zap, colorClass: 'text-amber-400', bgClass: 'bg-amber-500/10', dataKey: 'rps' },
        {
          label: '에러 발생률',
          value: `${(errorRate * 100).toFixed(2)}%`,
          icon: AlertCircle,
          colorClass: errorRate > HEALTH_THRESHOLDS.ERROR_RATE ? 'text-rose-500' : 'text-rose-400',
          bgClass: 'bg-rose-500/10',
          warning: errorRate > HEALTH_THRESHOLDS.ERROR_RATE,
          dataKey: 'error_rate',
        },
        { label: '평균 응답 시간', value: formatLatency(stats?.avg_latency_ms), icon: Clock, colorClass: 'text-emerald-400', bgClass: 'bg-emerald-500/10', dataKey: 'rps' },
      ];
    },
    [stats],
  );
                    ...
                    return (
                      <tr key={svc.name} className="hover:bg-slate-800/20 transition-colors">
                        ...
                        <td className="whitespace-nowrap px-4 py-4 font-mono text-sm text-slate-400 md:px-6 lg:px-8">
                          {timeSeries.length >= 2
                            ? ((svc.span_count / (timeSeries[timeSeries.length - 1].timestamp - timeSeries[0].timestamp)) || 0).toFixed(2)
