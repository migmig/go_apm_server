...
- [x] **T-038**: `web/` 디렉토리에 Vite 프로젝트 초기화 (`npm create vite@latest`)
- [x] **T-039**: `web/embed.go` - 빌드 결과물(`dist`)에 대한 `go:embed` 선언 및 서빙 핸들러 작성
- [x] **T-040**: 라우터, API 클라이언트 설정
- [x] **T-041**: 대시보드 페이지 (요청 수, 에러율 통계 및 최근 트레이스 목록)
- [x] **T-042**: 트레이스 목록 페이지 (검색 및 필터링 기능)
- [x] **T-043**: 트레이스 상세 페이지 (오픈소스 간트 차트 또는 Jaeger UI 컴포넌트를 연동한 워터폴 차트 구현) 및 스팬 속성 사이드 패널
- [x] **T-044**: 로그 뷰어 페이지 (로그 스트림, 검색/필터)
- [x] **T-045**: 메트릭 뷰 페이지 (시계열 차트 라이브러리 연동)
- [x] **T-046**: 프론트엔드 빌드 파이프라인을 Go 빌드 및 `Makefile`에 통합
...
- [x] **T-049**: Dockerfile 작성 (프론트엔드 빌드(Node) + Go 빌드 멀티스테이지)
- [x] **T-050**: Makefile 업데이트 (`build`, `run`, `docker-build`, `build-web` 추가)

## Phase 7: 테스트 및 검증 ✅

- [x] **T-051**: OTel SDK로 테스트 데이터 전송하는 예제 클라이언트 작성
- [x] **T-052**: storage 패키지 단위 테스트 (일별 파티셔닝 구조 검증)
- [x] **T-053**: API 핸들러 통합 테스트
- [x] **T-054**: Batch Processor 및 pdata 파싱 단위 테스트
- [x] **T-055**: 전체 E2E 테스트 (에이전트 → 배치 수신 → 일별 DB 저장 → API 조회 → UI 확인)
